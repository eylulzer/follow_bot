...
Follower::Follower() : imageTransport(nh) {
	imageSubscriber = imageTransport.subscribe("/d435i/color/image_raw", 1,
			&Follower::imageCallback, this);
	cmdVelPublisher = nh.advertise<geometry_msgs::Twist>("/dowaksa_amr_diff_drive_controller/cmd_vel", 5);
...
void Follower::imageCallback(const sensor_msgs::ImageConstPtr& msg) {
	...
	// cv::Scalar lower_yellow(9, 25, 19);
	// cv::Scalar upper_yellow(79, 228, 255);
